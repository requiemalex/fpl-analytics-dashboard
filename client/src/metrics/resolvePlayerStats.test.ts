...
describe("resolvePlayerStats — lastSeason mode", () => {
  it("returns every performance field null (<retained_not_omitted>) when there's no lastCompletedSeason entry", () => {
    ...
    const profile: HistoricPlayerProfile = { lastCompletedSeason: null, qualifyingSeasons: [], windowAverage: null, allSeasonsInWindow: [], floorSeasonsInWindow: [], floorWindowAverage: null };
  ...
  it("uses the historic season's own totals, and this app's estimatedPointsPerGame/perGame — never FPL's raw fields", () => {
    ...
    const profile: HistoricPlayerProfile = { lastCompletedSeason: season, qualifyingSeasons: [season], windowAverage: computeCareerAverages([season]), allSeasonsInWindow: [season], floorSeasonsInWindow: [], floorWindowAverage: null };
    const resolved = resolvePlayerStats(player, "lastSeason", profile, true);
...
describe("resolvePlayerStats — historicAverage mode", () => {
  ...
  it("uses windowAverage's per-season averages, defaulting count-type fields to 0 (not null) when the average itself is 0", () => {
    ...
    const profile: HistoricPlayerProfile = { lastCompletedSeason: seasons[1], qualifyingSeasons: seasons, windowAverage: avg, allSeasonsInWindow: seasons, floorSeasonsInWindow: [], floorWindowAverage: null };
    const resolved = resolvePlayerStats(player, "historicAverage", profile, true);
...
describe("resolvePlayerStatsList", () => {
  ...
  it("counts noDataCount correctly for a mixed set (some with data, some without)", () => {
    ...
    const profiles = new Map([[1, { lastCompletedSeason: season, qualifyingSeasons: [], windowAverage: null, allSeasonsInWindow: [], floorSeasonsInWindow: [], floorWindowAverage: null } as HistoricPlayerProfile]]);
...
describe("resolvePlayerStats — Historic Average games come from total minutes (audit 2026-09-25 V1)", () => {
  ...
  const profile: HistoricPlayerProfile = { lastCompletedSeason: seasons[3], qualifyingSeasons: [], windowAverage: computeCareerAverages(seasons), allSeasonsInWindow: seasons, floorSeasonsInWindow: [], floorWindowAverage: null };
  ...
  it("one season (Last Completed Season, Current Season) still uses that season's minutes", () => {
    ...
    const last: HistoricPlayerProfile = { lastCompletedSeason: one, qualifyingSeasons: [], windowAverage: computeCareerAverages([one]), allSeasonsInWindow: [one], floorSeasonsInWindow: [], floorWindowAverage: null };
