        ...
        <div>
          ...
          {hasSquadToShow && (
            <p className="page-subtitle">
              {active!.name}'s fixtures through to Gameweek {toGw}.
            </p>
          )}
        </div>
      </div>
        ...
        <>
          {squadPlayers.length < SQUAD_RULES.squadSize && (
            <div className="banner stale">
              "{active.name}" has {squadPlayers.length} of {SQUAD_RULES.squadSize} players — recommendations below (especially Bench
              Boost, which cares about all 15) will be based on an incomplete squad until you fill it out in Team Building.
            </div>
          )}
