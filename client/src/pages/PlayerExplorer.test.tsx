...
// Audit 2026-09-25 (Player Explorer), phase 1. Tests marked `it.fails` expose
// a finding from docs/audits/results/2026-09-25-player-explorer/1-audit.md and
// are expected to pass once that finding is fixed.
...
describe("Player Explorer — findings from the 2026-09-25 audit", () => {
  it.fails("H1: arriving from Teams (?team=1) shows the Team column's filter as active", () => {
    const { container } = renderExplorer("/players?team=1");
    expect(rowNames(container)).toEqual(["Zubimendi", "Ødegaard"]);
    expect(header(container, "Team").querySelector("button.column-filter-icon")?.className).toContain("active");
  });

  it.fails("H1: after arriving from Teams, choosing another club in the Team column shows that club's players", () => {
    const { container } = renderExplorer("/players?team=1");
    applyCategoryFilter(container, "Team", "CHE");
    expect(rowNames(container)).toEqual(["Ángel", "Bob"]);
  });

  it.fails("M2: 'Equal to' matches a value typed exactly as the table displays it", () => {
    const { container } = renderExplorer();
  ...
  it.fails("L3: sorting by Player A→Z is alphabetical, accented names included", () => {
    const { container } = renderExplorer();
